...
public class ProductManager : IProductService
{
    ...
    public async Task<ResponseDto<ProductDto>> AddAsync(ProductCreateDto productCreateDto)
    {
        try
        {
            ...
            {
                return ResponseDto<ProductDto>.Fail("En az bir tane kategori Id seçilmedilir", StatusCodes.Status400BadRequest);
            }
    ...
    public async Task<ResponseDto<NoContent>> HardDeleteAsync(int id)
    {
        try
        {
            ...
            if (result < 1)
            {
                return ResponseDto<NoContent>.Fail("ürün silenirken bir hata oluştu", StatusCodes.Status500InternalServerError);
            }
            var deleteResult = _imageService.DeleteImage(product.ImageUrl!);
