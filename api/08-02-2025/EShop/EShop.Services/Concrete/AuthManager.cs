...
public class AuthManager : IAuthService
{
    ...
    public async Task<ResponseDto<NoContent>> ChangePasswordAsync(ChangePasswordDto changePasswordDto)
    {
        try
        {
            ...
            if (!result.Succeeded)
            {
                return ResponseDto<NoContent>.Fail(result.Errors.Select(x => x.Description).ToList(), StatusCodes.Status400BadRequest);
            }
    ...
    public async Task<ResponseDto<NoContent>> ForgotPasswordAsync(ForgotPasswordDto forgotPasswordDto)
    {
        try
        {
            ...
            var htmlBody=$"<p>Şifrenizi sıfırlamak için aşşağıdaki linke tıklayınız:</p><a href='{resetLink}'>Şifremi Değiştir</a>";
          var response=  await _emailManager.SendEmailAsync(
                forgotPasswordDto.Email!,
                "Esop şifre değiştirme talebi",
                htmlBody
            );
    ...
    public async Task<ResponseDto<NoContent>> ResetPasswordAsync(ResetPasswordDto resetPasswordDto)
    {
        try
        {
            ...
            if (!result.Succeeded)
            {
                return ResponseDto<NoContent>.Fail("Şifre sıfırlanırken sorun oluştu!",StatusCodes.Status400BadRequest);
            }
            return ResponseDto<NoContent>.Success(StatusCodes.Status200OK);
        }
        catch (System.Exception ex)
